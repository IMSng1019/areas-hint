...
- `/areahint check <联合域名>` - 显示指定联合域名的详细信息，包括所有域名和一级顶点（权限等级：0，普通玩家可用）
...
Check指令提供了查看联合域名信息的便捷方式，允许玩家快速了解当前维度中的所有联合域名以及每个联合域名包含的具体域名详情。

### 主要特点

- **权限开放**：普通玩家即可使用，无需管理员权限（权限等级：0）
- **智能显示**：优先显示联合域名，如无设置则显示实际域名
- **交互界面**：支持点击联合域名查看详细信息
- **自动补全**：支持联合域名的智能补全功能
- **详细信息**：显示域名的所有属性，包括一级顶点坐标
...
显示当前维度中的所有联合域名列表，每个联合域名都可以点击查看详细信息。
...
>   高度范围: 64 到 100
>   一级顶点数量: 4
>   顶点坐标: (100, 200), (150, 200), (150, 250)...
> 
> 域名 #2:
>   实际域名: shop_area_south
>   联合域名: 商店区
>   域名等级: 2级
>   ...
```
...
#### 联合域名处理
- 使用 `SurfaceNameHandler` 工具类处理显示逻辑
- 优先显示 `surfacename` 字段，回退到 `name` 字段
- 自动分组相同联合域名的多个域名
...
域名扩展（ExpandArea）系统允许玩家对现有域名进行几何扩展，通过添加新的顶点区域来扩大原有域名的覆盖范围。该系统包含复杂的几何算法来处理边界点计算、交叉点检测和顶点重新排序。

### 核心特性

- **权限验证**：只有域名创建者、basename引用的玩家或管理员才能扩展域名
- **交互式记录**：通过X键记录新区域顶点，Enter键确认完成
- **几何算法**：自动计算边界点、处理交叉线段、删除内部顶点
- **高度验证**：确保新添加区域的高度在合理范围内
- **实时同步**：扩展完成后自动同步给所有在线玩家

### 使用流程

1. **启动扩展**：执行 `/areahint expandarea` 指令
2. **选择域名**：在弹出界面中输入要扩展的域名名称
3. **权限检查**：系统自动验证玩家权限
4. **域名选择**：从可修改的域名列表中选择目标域名
5. **记录顶点**：按X键记录新区域的顶点位置（至少3个）
6. **确认扩展**：按Enter键完成记录并处理扩展
...
- **普通玩家**：只能扩展自己创建的域名或basename引用自己的域名
- **管理员**：可以扩展任何域名
- **权限等级**：需要权限等级0（不需要管理员权限）

### 实现架构

#### 客户端组件
- `ExpandAreaManager.java`：核心管理器，处理流程控制
- `ExpandAreaUI.java`：用户界面系统
- `GeometryCalculator.java`：几何计算算法
- `ExpandAreaKeyHandler.java`：按键处理器

#### 服务端组件
- `ExpandAreaServerNetworking.java`：网络通信处理
- 集成到 `ServerCommands.java` 的指令处理

#### 网络通信
- `EXPAND_AREA_CHANNEL`：客户端到服务端的扩展请求
- `EXPAND_AREA_RESPONSE_CHANNEL`：服务端到客户端的响应
...
域名收缩（ShrinkArea）系统允许玩家对现有域名进行几何收缩，通过标记需要删除的区域来缩小原有域名的覆盖范围。该系统实现了复杂的几何算法来处理边界点计算、线段交叉检测和顶点重新排序。

### 核心特性

- **权限验证**：只有域名创建者、basename引用的玩家或管理员才能收缩域名
- **交互式记录**：通过X键记录收缩区域顶点，自动检测并结束记录
- **复杂几何算法**：自动计算边界点、处理交叉线段、删除内部顶点
- **智能边界处理**：计算收缩区域与原域名的交点作为新边界
- **实时同步**：收缩完成后自动同步给所有在线玩家

### 使用流程

1. **启动收缩**：执行 `/areahint shrinkarea` 指令
2. **选择域名**：在弹出界面中选择要收缩的域名
3. **权限检查**：系统自动验证玩家权限
4. **记录顶点**：按X键记录收缩区域的顶点位置（至少3个）
5. **确认收缩**：再次按X键完成记录并处理收缩

### 实现架构

#### 客户端组件
- `ShrinkAreaManager.java`：核心管理器，处理流程控制和状态管理
- `ShrinkAreaUI.java`：用户界面系统，提供域名选择界面
- `ShrinkGeometryCalculator.java`：复杂几何计算算法实现
- `ShrinkAreaKeyHandler.java`：X键按键处理器

#### 服务端组件
- `ShrinkAreaServerNetworking.java`：网络通信处理和权限验证
- 集成到 `ServerCommands.java` 的指令处理

#### 网络通信
- `SHRINK_AREA_CHANNEL`：客户端到服务端的收缩请求
- `SHRINK_AREA_RESPONSE_CHANNEL`：服务端到客户端的响应

### 权限系统

- **普通玩家**：只能收缩自己创建的域名或basename引用自己的域名
- **管理员**：可以收缩任何域名（权限等级2）
- **目标选择器**：作用于玩家自己
- **权限等级**：0（不需要管理员权限）

此系统为域名管理提供了精确的几何收缩能力，允许玩家根据需要精细调整域名边界，特别适用于复杂地形的域名优化。 