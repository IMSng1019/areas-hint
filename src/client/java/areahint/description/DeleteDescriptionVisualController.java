...
/**
 * deletedescription 图形流程控制器，只把界面选择转换为现有描述删除流程。
 */
public final class DeleteDescriptionVisualController {
    ...
    private DeleteDescriptionVisualController() {
    }
    ...
    public static void openFromCommandUi(Screen parent) {
        ...
        CommandUiActions.runCommandAndClose(parent, "areahint deletedescription");
    }
    ...
    public static void showFirstConfirm(DescriptionListEntry entry) {
        setScreen(new WizardConfirmScreen(parentScreen,
            "commandui.deletedescription.title",
            I18nManager.translate("description.ui.confirm.target", title(entry)),
            List.of(I18nManager.translate("description.ui.confirm.delete.warning"), detail(entry)),
            "commandui.button.continue",
            () -> CommandUiActions.runCommand("areahint deletedescription confirm"),
            DescriptionManager.getInstance()::cancel));
    }

    public static void showSecondConfirm(DescriptionListEntry entry) {
        setScreen(new WizardConfirmScreen(parentScreen,
            "commandui.deletedescription.title",
            I18nManager.translate("description.ui.confirm.delete.second.warning"),
            List.of(I18nManager.translate("description.ui.confirm.target", title(entry)), detail(entry)),
            "commandui.button.delete",
            () -> CommandUiActions.runCommand("areahint deletedescription confirm2"),
            DescriptionManager.getInstance()::cancel));
    }
    ...
    private static String detail(DescriptionListEntry entry) {
        return I18nManager.translate("description.ui.hover.realname") + nullToText(entry.id()) + "  "
            + I18nManager.translate("description.ui.hover.level") + entry.level() + "  "
