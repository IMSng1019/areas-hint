...
public class DivideAreaUI {
    ...
    public void showPointRecordedOptions(int count) {
        ...
        MutableText saveBtn = Text.literal("§b[确认分割]")
            .setStyle(Style.EMPTY
                .withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/areahint dividearea save"))
                .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Text.of("确认分割")))
                .withColor(Formatting.AQUA));

        if (count >= 2) {
            client.player.sendMessage(Text.empty().append(continueBtn).append(Text.of("  ")).append(saveBtn), false);
        } else {
            client.player.sendMessage(continueBtn, false);
